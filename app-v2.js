...
class MFCOrdersApp {
    ...
    initEventListeners() {
        ...
        document.getElementById('exportProductsCSV').addEventListener('click', () => this.exportProductsCSV());
        document.getElementById('importProductsCSV').addEventListener('click', () => document.getElementById('productCSVInput').click());
        document.getElementById('productCSVInput').addEventListener('change', (e) => this.importProductsCSV(e));
    ...
    exportProductsCSV() {
        ...
        const headers = ['ID', 'SKU', 'Name', 'Category', 'Price', 'Box Size', 'Active'];
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `mfc-products-${new Date().toISOString().split('T')[0]}.csv`;
        a.click();
        URL.revokeObjectURL(url);

        alert(`Exported ${this.products.length} products to CSV`);
    }
    ...
    importProductsCSV(e) {
        ...
        reader.onload = (event) => {
            try {
                ...
                const replace = confirm(
                    `Found ${importedProducts.length} products in CSV.\n\n` +
                    `OK = REPLACE all current products (${this.products.length})\n` +
                    `Cancel = MERGE (update existing, add new)`
                );

                if (replace) {
                    this.products = importedProducts;
                    alert(`Replaced all products with ${importedProducts.length} from CSV`);
                } else {
                    let updated = 0;
                    let added = 0;

                    importedProducts.forEach(imported => {
                        const existing = this.products.find(p => p.id === imported.id);
                        if (existing) {
                            Object.assign(existing, imported);
                            updated++;
                        } else {
