...
My Father Cigars Southeast`;
    ...
    async loadYoYData() {
        try {
            const response = await fetch('/yoy_comparison_2024_2025.json');
            this.yoyData = await response.json();
            this.renderYoYComparison();
        } catch (error) {
            console.log('YoY data not available:', error);
            // Data file not uploaded yet - that's okay
        }
    }
