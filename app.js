...
My Father Cigars Southeast`;
    ...
    exportTop10PDF() {
        ...
        // Create chart data for each table
        const createChartData = (skus, title, color) => {
            if (!skus || skus.length === 0) return '';
            
            const labels = skus.map(s => s.name.length > 30 ? s.name.substring(0, 30) + '...' : s.name);
            const quantities = skus.map(s => s.quantity);
            const percentages = skus.map(s => parseFloat(s.percentage));
        ...
        // Territory chart
        charts.push(createChartData(data.territory, 'Territory Top 10', '#3b82f6'));
        
        // State chart
        if (data.state && data.state.length > 0) {
            charts.push(createChartData(data.state, `${data.customerState} Top 10`, '#10b981'));
        }
        
        // Metro chart
        if (data.metro && data.metro.length > 0) {
            charts.push(createChartData(data.metro, `${data.customerCity} Metro Top 10`, '#ef4444'));
        }
        
        // Account chart
        if (data.account && data.account.length > 0) {
            charts.push(createChartData(data.account, `${data.customerName} - All-Time`, '#a78bfa'));
        }
        
        // Recent chart
        if (data.recent && data.recent.length > 0) {
            charts.push(createChartData(data.recent, `${data.customerName} - Last 6 Months`, '#fbbf24'));
        }
            ...
            <body>
                <div class="header">
                    ...
                    <div class="meta">
                        ...
                        ${data.customerState ? ` | State: ${data.customerState}` : ''}
                        ${data.customerCity ? ` | Metro: ${data.customerCity}` : ''}
                    </div>
                </div>
                                    ...
                                    tooltip: {
                                        callbacks: {
                                            label: function(context) {
                                                ...
                                                const pct = chartData.percentages[context.dataIndex];
                                                return 'Qty: ' + qty + ' (' + pct + '%)';
                                            }
                                        }
                                    }
