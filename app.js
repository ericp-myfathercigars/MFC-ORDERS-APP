...
My Father Cigars Southeast`;
    ...
    renderTop10Tables(searchedCustomer) {
        ...
        this.historicalOrders.forEach(order => {
            const current = customerTotals.get(order.customerName) || { 
                revenue: 0, 
                orders: 0,
                state: '',
                city: ''
            ...
            // Get state and city from customers list
            const cust = this.customers.find(c => c.name === order.customerName);
            if (cust) {
                current.state = cust.shipState;
                current.city = cust.shipCity;
            }
            
            customerTotals.set(order.customerName, current);
        });
        
        // Convert to array and sort
        const allCustomers = Array.from(customerTotals.entries()).map(([name, data]) => ({
            name,
        ...
        // 1. Territory Top 10
        const territoryTop10 = allCustomers.slice(0, 10);
        
        // 2. State Top 10 (if customer has a state)
        const stateTop10 = customerState 
            ? allCustomers.filter(c => c.state === customerState).slice(0, 10)
            : [];
        
        // 3. Metro Area Top 10 (same city)
        const metroTop10 = customerCity
            ? allCustomers.filter(c => c.city === customerCity).slice(0, 10)
            : [];
        
        // 4. Searched Account ranking
        const searchedAccount = allCustomers.find(c => c.name === searchedCustomer);
        const accountRank = allCustomers.findIndex(c => c.name === searchedCustomer) + 1;
        
        // Render tables
        container.innerHTML = `
            <div class="top10-grid">
                ${this.renderTop10Table('Territory Top 10', territoryTop10, 'territory')}
                ${customerState ? this.renderTop10Table(`${customerState} Top 10`, stateTop10, 'state') : ''}
                ${customerCity ? this.renderTop10Table(`${customerCity} Metro Top 10`, metroTop10, 'metro') : ''}
                ${searchedAccount ? this.renderAccountCard(searchedAccount, accountRank) : ''}
            </div>
        `;
    }
    
    renderTop10Table(title, customers, colorClass) {
                ...
                <table class="top10-table">
                    <thead>
                        <tr>
                            ...
                            <th class="top10-customer">Customer</th>
                            <th class="top10-orders">Orders</th>
                    ...
                    <tbody>
                        ${customers.map((customer, index) => `
                            <tr>
                                ...
                                <td class="top10-customer">${customer.name}</td>
                                <td class="top10-orders">${customer.orders}</td>
                                <td class="top10-revenue">${this.formatCurrency(customer.revenue)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    exportTop10PDF() {
        ...
        printWindow.document.write(`
            <html>
            <head>
                <title>Top 10 Customers - Historical Performance</title>
                <style>
                    ...
                    .top10-orders { text-align: center; }
                    @media print {
            ...
            <body>
                <h1>Top 10 Customers - Historical Performance</h1>
